...
function getAdminUsers(){
	global $db, $roles;
	$sql = "SELECT * FROM blog_users WHERE role IS NOT NULL";
...
function createAdmin($request_values){
	...
	$user_check_query = "SELECT * FROM blog_users WHERE username='$username' 
	...
	if (count($errors) == 0) {
		...
		$query = "INSERT INTO blog_users (username, email, role, password, created_at, updated_at) 
...
function editAdmin($admin_id)
{
	...
	$sql = "SELECT * FROM blog_users WHERE id=$admin_id LIMIT 1";
	$result = mysqli_query($db, $sql);
...
function updateAdmin($request_values){
	...
	if (count($errors) == 0) {
		...
		$query = "UPDATE blog_users SET username='$username', email='$email', role='$role', password='$password' WHERE id=$admin_id";
		mysqli_query($db, $query);
...
function deleteAdmin($admin_id) {
	global $db;
	$sql = "DELETE FROM blog_users WHERE id=$admin_id";
...
function createTopic($request_values){
	...
	$topic_check_query = "SELECT * FROM blog_topic WHERE slug='$topic_slug' LIMIT 1";
	...
	if (count($errors) == 0) {
		$query = "INSERT INTO blog_topic (name, slug) 
...
function editTopic($topic_id) {
	...
	$sql = "SELECT * FROM blog_topic WHERE id=$topic_id LIMIT 1";
	$result = mysqli_query($db, $sql);
...
function updateTopic($request_values) {
	...
	// register topic if there are no errors in the form
	if (count($errors) == 0) {
		$query = "UPDATE blog_topic SET name='$topic_name', slug='$topic_slug' WHERE id=$topic_id";
		mysqli_query($db, $query);
...
function deleteTopic($topic_id) {
	global $db;
	$sql = "DELETE FROM blog_topic WHERE id=$topic_id";
	if (mysqli_query($db, $sql)) {
